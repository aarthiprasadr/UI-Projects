angular
		.module('bfApp.controllers', [])
		.controller(
				'AllDeals',
				function($scope, $rootScope, bfService) {
					...
					$scope.searchFilter = function(deal) {
						...
						var result = (!$scope.nameFilter || re.test(deal.item)
								...
								|| re.test(deal.rebate) || re.test(deal.price));
		...
		controller(
				...
				function($scope, $route, $location, $routeParams, bfService) {
					...
					bfService.getDealsByStore(store).success(
							function(response) {
								$scope.dealList = response.deals;
							});

					bfService
							.getCategoriesByStore(store)
							.success(
									function(response) {
